...
function UserSettings() {
    ...
    const deleteAccount = () => {
        if (localStorage.getItem('authorization')) {
            ...
            axios.post('https://dev.enzo-salson.fr/api/deleteUser', {}, {headers}).then(() => {
    ...
    useEffect(() => {
        if (localStorage.getItem("authorization")) {
            ...
            axios
                .post("https://dev.enzo-salson.fr/api/getUnameMailBio", {}, { headers })
                .then((response) => {
                    setImageUrl(`https://dev.enzo-salson.fr/api/getProfilePic?id=${response.data.id}`);
                });
        }
    }, []);
        ...
        >
            ...
            <div className="flex border border-gray-300 rounded m-w-fit">
                ...
                <input
                    ...
                    className={`cursor-pointer text-white flex justify-center rounded items-center min-w-[8rem] min-h-[2.5rem] gap-2 px-2 bg-[#310046] hover:bg-[#510273]`}
                />
            </div>
            <div className="flex flex-col justify-center h-32 overflow-hidden">
                {imageUrl ? <img crossOrigin='anonymous' className="h-full w-auto object-contain" src={imageUrl} alt="Profile" /> : null}
            </div>
        </form>
    );
}
